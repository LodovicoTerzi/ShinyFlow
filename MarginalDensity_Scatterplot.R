marginal_plot = function(x, y, group = NULL, data = NULL, lm_show = FALSE, lm_formula = y ~ x, bw = "nrd0", adjust = 1, alpha = 1, plot_legend = T, ...){
  ...
  if(missing(group)){
    ...
    group_colors = c("midnightblue", "orange")
  ...
  tryCatch(expr = {
    ...
    mapply(function(group_set, group_color){
      lines(density(group_set$x, bw = bw, adjust = adjust), col = group_color, lwd = 2)
      polygon(density(group_set$x, bw = bw, adjust = adjust), col = adjustcolor(group_color, alpha.f = 0.7))
    ...
    par(mar = c(4,5,0,0))
    if(missing(group)){
      do.call(plot, c(list(x = quote(data$x), y = quote(data$y), col = quote(scales::alpha("black", alpha))), moreargs, pch=16))
    } else {
      do.call(plot, c(list(x = quote(data$x), y = quote(data$y), col = quote(scales::alpha(group_colors[data$group], alpha))), moreargs, pch=16))
    }
    ...
    mapply(function(group_set, group_color){
      lines(x = density(group_set$y, bw = bw, adjust = adjust)$y, y = density(group_set$y, bw = bw)$x, col = group_color, lwd = 2)
      polygon(x = density(group_set$y, bw = bw, adjust = adjust)$y, y = density(group_set$y, bw = bw)$x, col = adjustcolor(group_color, alpha.f = 0.7))
    }, data_split, group_colors)
